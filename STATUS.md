...
**Still not done:** `createGroupBookingAction` is still on mock (see the read/write-split entry above) — extending it to accept per-room dates would be the natural point to also wire in per-room availability display on that form.
