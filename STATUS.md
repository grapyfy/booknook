...
**Still to do from this list (not started):** tax-inclusive total shown live during booking (needs the GST-config wiring above first, so the form reads real configured rates instead of hardcoding a duplicate 12/18/7500), check-in/out time (contract change, needs Gautam's sign-off before either side builds), ID+photo capture at check-in (needs Supabase Storage bucket — no file storage exists in this project yet), undo for check-in/checkout (status-revert only, per the confirmed scope), email system (plumbing only — per CLAUDE.md, no live send until content is reviewed), back buttons + UI polish items (Gautam's side).
