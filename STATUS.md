...
**Not done:** the booking calendar's drag-and-drop reschedule doesn't yet show a live availability hint before you drop (it'll still get rejected server-side by `rescheduleBooking`'s new check if the target is unavailable, just without the preview). `createGroupBookingAction` is still on mock (see the read/write-split entry above) — extending it to accept per-room dates would be the natural point to also wire in per-room availability display on that form.
