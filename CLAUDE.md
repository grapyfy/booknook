...
**Branch size note:** this branch is now 10 STATUS.md entries / 5 commits from one continuous session — exactly the "grown large, ship in smaller pieces" signal this doc's own engineering standards call out below. Worth reviewing and merging before it grows further, rather than treating this as the new normal size for a single branch. Whoever reviews this branch should know all of the above going in.
